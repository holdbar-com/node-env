...
export class Changes {
  ...
  async postCompile(
    ...
    const result = (
      await Promise.all([
        compileResult,
        this.#ifChanged('formatting', source, (s) =>
          formatted(reporter, path, s)
        ),
        this.#ifChanged('spelling', source, (s) => spelling(reporter, path, s)),
        this.#ifChanged('linting', source, (s) =>
          lint(reporter, path, s, this.#lintCache)
        ),
  ...
  async shouldInstall() {
    ...
    const latestPackage =
      (
        await Promise.all(
          ['package.json', 'package-lock.json'].map((f) =>
            stat(resolve(this.#path, f))
          )
        )
      )
        .map((s) => s.ctimeMs)
  ...
  async #ifChanged(
    ...
    if (stages[stage]) {
      const lastSuccess = new Date(
        this.#timestamps.stages[stage] ?? 0
      ).getTime();
...
async function loadTimestamps(path: string) {
  try {
    return JSON.parse(
      await readFile(join(path, '.timestamps.json'), 'utf-8')
    ) as Timestamps;
  } catch (e) {
    if (isFileNotFound(e)) {
      return {
